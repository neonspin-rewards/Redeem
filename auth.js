...
/**
 * Check if current user is admin (case-insensitive email comparison)
 * @returns {boolean} True if user is admin
 */
export const isAdmin = () => {
  if (!_currentUser?.email) {
    console.log('[auth] isAdmin: No user logged in');
    return false;
  }
  
  const userEmail = _currentUser.email.toLowerCase().trim();
...
export function initAuthObserver(onUserReady, onUserGone) {
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      // Check admin status and show admin panel link
      _updateAdminPanelVisibility();
...
/* ── Admin Panel Visibility ───────────────────────────────────────
   Shows/hides admin panel link based on user's admin status.
   Admin link is injected dynamically into navigation when needed.
   ------------------------------------------------------------------ */
function _updateAdminPanelVisibility() {
  if (!isAdmin()) {
    console.log('[auth] User is not admin, hiding admin panel');
    _hideAdminPanel();
    return;
  }
  
  console.log('[auth] ✅ Admin user detected! Showing admin panel link');
  
  // Find or create admin link in navigation
  let adminLink = document.getElementById('admin-nav-link');
  
  if (!adminLink) {
    // Create admin link dynamically
    ...
    if (nav) {
      ...
      adminLink.addEventListener('mouseenter', () => {
        ...
        adminLink.style.boxShadow = '0 6px 16px rgba(255, 0, 128, 0.4)';
      });
      adminLink.addEventListener('mouseleave', () => {
        adminLink.style.transform = 'translateY(0)';
        adminLink.style.boxShadow = '0 4px 12px rgba(255, 0, 128, 0.3)';
      });
      
      nav.appendChild(adminLink);
      console.log('[auth] ✅ Admin link added to navigation');
    } else {
      console.warn('[auth] ⚠️ Could not find navigation element to add admin link');
    }
  } else {
    // Link already exists, just show it
    adminLink.classList.remove('hidden');
    adminLink.style.display = 'inline-flex';
    console.log('[auth] ✅ Admin link shown');
  }
}
...
function _hideAdminPanel() {
  ...
  if (adminLink) {
    ...
    console.log('[auth] Admin link hidden');
  }
}
