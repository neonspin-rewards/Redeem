...
async function checkAdminAccess(user) {
  console.log('[AdminAccess] Checking admin access for user:', {
    uid: user.uid,
    email: user.email,
  ...
  try {
    ...
    console.log('[AdminAccess] Fetching user data from:', userRef.toString());
    
    const snap = await get(userRef);
    const userData = snap.exists() ? snap.val() : {};
    
    console.log('[AdminAccess] User data retrieved:', {
      exists: snap.exists(),
      role: userData.role,
    ...
    if (user.email === SUPER_ADMIN_EMAIL) {
      console.log('[AdminAccess] Email matches SUPER_ADMIN_EMAIL, granting super_admin role');
      role = 'super_admin';
      // Also write it to DB if not already set
      if (userData.role !== 'super_admin') {
        console.log('[AdminAccess] Updating user role in database to super_admin');
        await update(ref(db, `users/${user.uid}`), { role: 'super_admin' }).catch((e) => {
          console.error('[AdminAccess] Failed to update super_admin role:', e);
        });
      }
    }

    console.log('[AdminAccess] Final role determination:', role);
    
    if (role === 'admin' || role === 'super_admin') {
      _currentAdminRole  = role;
      _currentAdminId    = user.uid;
      _currentAdminEmail = user.email;
      console.log('[AdminAccess] ✅ Access GRANTED:', { role, uid: user.uid, email: user.email });
      return { allowed: true, role };
    }

    console.log('[AdminAccess] ❌ Access DENIED: User role is', role);
    return { allowed: false, role };
  } catch (e) {
    console.error('[AdminAccess] ❌ Error checking role:', e);
    console.error('[AdminAccess] Error details:', {
      code: e.code,
      message: e.message,
      stack: e.stack
    });
    ...
    if (user.email === SUPER_ADMIN_EMAIL) {
      console.log('[AdminAccess] Fallback: Email matches SUPER_ADMIN_EMAIL, granting super_admin');
...
async function updateUserCoins(uid, amount, operation) {
  console.log('[Admin] Starting coin update:', { uid, amount, operation });
  
  try {
    _guardAdminTarget(uid);
    console.log('[Admin] Admin target guard passed');
    
    const userRef = ref(db, `users/${uid}`);
    console.log('[Admin] Fetching user data from:', userRef.toString());
    ...
    console.log('[Admin] Coin update calculation:', {
      current,
      operation,
      amount,
      newVal
    ...
    console.log('[Admin] Updating user coins in database...');
    await update(userRef, { coins: newVal });
    ...
    console.error('[Admin] ❌ Coin update failed:', e);
    console.error('[Admin] Error details:', {
      code: e.code,
      message: e.message,
      permission: e.code === 'PERMISSION_DENIED' ? 'Permission denied - check Firebase rules' : 'Other error',
      stack: e.stack
    });
...
async function updateUserExp(uid, amount, operation) {
  console.log('[Admin] Starting EXP update:', { uid, amount, operation });
  ...
  try {
    ...
    console.log('[Admin] EXP update calculation:', {
      current,
      operation,
      amount,
      newVal
    ...
    console.log('[Admin] Updating user EXP in database...');
    await update(userRef, { exp: newVal });
    console.log('[Admin] ✅ Database update successful');
    ...
    console.error('[Admin] ❌ EXP update failed:', e);
    console.error('[Admin] Error details:', {
      code: e.code,
      message: e.message,
      permission: e.code === 'PERMISSION_DENIED' ? 'Permission denied - check Firebase rules' : 'Other error',
...
async function banUser(uid) {
  console.log('[Admin] Starting ban operation for user:', uid);
  
  try {
    _guardAdminTarget(uid);
    console.log('[Admin] Admin target guard passed, proceeding with ban');
    
    const userRef = ref(db, `users/${uid}`);
    console.log('[Admin] Updating user at path:', userRef.toString());
    
    await update(userRef, { banned: true, bannedAt: Date.now() });
    console.log('[Admin] ✅ Database update successful');
    ...
    console.error('[Admin] ❌ Ban operation failed:', e);
    console.error('[Admin] Error details:', {
      code: e.code,
      message: e.message,
      permission: e.code === 'PERMISSION_DENIED' ? 'Permission denied - check Firebase rules' : 'Other error',
...
document.getElementById('btn-send-redeem')?.addEventListener('click', async () => {
  ...
  console.log('[Admin] Send redeem clicked:', { uidOrEmail, note });
  
  if (!uidOrEmail || !note) { 
    console.warn('[Admin] Missing required fields');
    _toast('⚠️ Fill in all fields.'); 
    return; 
  }
  ...
  if (!target) { 
    console.error('[Admin] User not found:', uidOrEmail);
    _toast('❌ User not found. Check UID or email.'); 
    return; 
  }

  console.log('[Admin] Target user found:', {
    uid: target.uid,
    email: target.email,
  ...
  try {
    const redeemRef = ref(db, `users/${target.uid}/redeems`);
    console.log('[Admin] Pushing redeem to:', redeemRef.toString());
    ...
    console.log('[Admin] ✅ Redeem sent successfully');
    ...
    console.error('[Admin] ❌ Send redeem failed:', e);
    console.error('[Admin] Error details:', {
      code: e.code,
      message: e.message,
      permission: e.code === 'PERMISSION_DENIED' ? 'Permission denied - check Firebase rules' : 'Other error',
 ...
 * Example URL: https://res.cloudinary.com/dary8yynb/image/upload/v1234567890/neonspin_upload/abc123.jpg
 * Returns: neonspin_upload/abc123
 */
function _extractCloudinaryPublicId(imageUrl) {
  if (!imageUrl) return null;
  try {
    // Match pattern: /upload/v{timestamp}/{folder}/{filename}
    const match = imageUrl.match(/\/upload\/v\d+\/(.+)\.\w+$/);
    if (match && match[1]) {
      console.log('[Cloudinary] Extracted public_id:', match[1]);
      return match[1];
    }
    // Alternative pattern without version: /upload/{folder}/{filename}
    const altMatch = imageUrl.match(/\/upload\/([^\/]+\/[^\/]+)\.\w+$/);
    if (altMatch && altMatch[1]) {
      console.log('[Cloudinary] Extracted public_id (alt):', altMatch[1]);
      return altMatch[1];
    }
  } catch (e) {
    console.error('[Cloudinary] Error extracting public_id:', e);
  }
  return null;
}
 ...
 * Note: This requires a server-side endpoint or Cloudinary upload preset with delete permissions
 * For now, we'll log the deletion attempt and remove from DB
 */
async function _deleteCloudinaryImage(imageUrl) {
  const publicId = _extractCloudinaryPublicId(imageUrl);
  if (!publicId) {
  ...
  // NOTE: Direct deletion from client requires signed requests or admin API
  // This is a security limitation - images should be deleted server-side
  // For now, we log the deletion and remove from DB
  // You would need a Cloud Function or backend API to actually delete the image
  
  try {
    // If you have a backend API endpoint for deletion, call it here:
    // const response = await fetch('YOUR_BACKEND_URL/delete-image', {
    //   method: 'POST',
    ...
    console.warn('[Cloudinary] Note: Client-side deletion is not supported. Image remains in Cloudinary.');
    console.warn('[Cloudinary] To fully delete images, implement a server-side deletion endpoint.');
    return true;
  } catch (e) {
...
async function _loadFeedback() {
  ...
  tbody.innerHTML = rows.slice(0, 100).map(f => `
    <tr>
      ...
      <td>
        <button class="btn btn-red" onclick="window._deleteFeedback('${f.key}', ${f.imageURL ? `'${f.imageURL}'` : 'null'})">🗑 Delete</button>
      </td>
    </tr>
  `).join('');
}

window._deleteFeedback = async function(key, imageUrl = null) {
  const ok = await _confirmAction('🗑 Delete Feedback', 'Delete this feedback item permanently?');
  if (!ok) return;
  
  console.log('[Admin] Deleting feedback:', { key, imageUrl });
  
  try {
    // If there's an image, attempt to delete it from Cloudinary first
    if (imageUrl) {
      console.log('[Admin] Feedback has image, attempting Cloudinary deletion...');
      await _deleteCloudinaryImage(imageUrl);
    }
    
    // Delete from Firebase RTDB
    console.log('[Admin] Removing feedback from database...');
    await remove(ref(db, `feedback/${key}`));
    
    await logAdminAction('DELETE_FEEDBACK', '', `Feedback deleted: key=${key}${imageUrl ? ' (with image)' : ''}`);
    console.log('[Admin] Feedback deleted successfully');
    
    _toast('🗑 Feedback deleted.');
    _loadFeedback();
  } catch (e) {
    console.error('[Admin] Feedback deletion error:', e);
    console.error('[Admin] Error details:', {
      code: e.code,
      message: e.message,
      stack: e.stack
